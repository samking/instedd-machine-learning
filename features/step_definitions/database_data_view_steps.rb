...
When /^the contents of the #{QUOTED_ARG} csv file is in the database$/ do |filename|
  ...
  assert(@test_client[:dataset].database_table.has_elements?(elements), 
         "Some element(s) didn't make it into the database.  The elemets 
         were: #{elements}.  The database contents: 
         #{@test_client[:dataset].database_table.as_map}")
end
